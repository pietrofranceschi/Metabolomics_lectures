...
* during this time `r length(raw_one)` spectra were collected
...
spectra_tibble <- tibble(spc_id = 1:length(rt)) %>% 
  mutate(rtime = map_dbl(spc_id, ~ rt[.x])) %>% 
...
ggplotly(spectra_tibble  %>% unnest(data) %>% 
  filter(mz > 284 & mz < 300) %>% 
  filter(rtime > 200 & rtime < 300) %>% 
  ggplot() + 
  geom_point(aes(x = rtime, y = mz, col = log10(I), size = I)) + 
