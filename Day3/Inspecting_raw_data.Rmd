...
ggplotly(tibble(rt = rt, mz = mz, I = I)  %>% 
  unnest(c("mz","I")) %>%
  filter(between(mz,280,310)) %>% 
  filter(between(rt,200,300)) %>% 
  ggplot() + 
...
We need to find the spectrum number which is close to 250 seconds ...


```{r}
ggplotly(fs_raw[[193]] %>% plot())
```
