...
Here we have a tricky situation. The peaks are not aligned, but this is actually a double feature, which is also potential biomarker for color and variety ... so we should go back to peak detection and play around ;-)

Anyway, let's try to correct the rt shift. As usual in xcms this can be done in different way, here we will use the most simple solution (a potentially more flexible one based on *dynamic time warping* is also available) 
...
The idea behind retention time is quite simple:
...
NAs will be discussed in more details by Maria in the afternoon, here we can anticipate that the two previous processes will result in different type of missingness (either random or non random) ...
...
The situation improved, but it is not yet perfect. Wait until the afternoon to see what else can be done.


## Checking Feature chromatograms

A typical thing one wants to at some point in the analysis is to check if the chromatographic peaks of a really interesting feature are really there ...

In order to do that xcms provides a really handy function

```{r}
ft_chr1 <- featureChromatograms(raw_data, features = 10, 
...
* If you go back to the feature tables, you will see that in some cases the number of peaks assigned to a features can be larger than the number of samples. How can this be? Try to check the diagnostic plots and understand what happens
