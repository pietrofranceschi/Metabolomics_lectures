...
```{r}
plot(mz$F1.S0130,I$F1.S0130, type = "h", main = names(mz)[1])
```
...
```{r}
mytibble <- tibble(rt = by_file_rt[[1]], mz = by_file_mz[[1]], I = by_file_I[[1]])
mytibble
```
...
test <- mytibble %>% 
  unnest(c("mz","I")) %>% 
  ggplot() + 
  geom_point(aes(x = rt, y = mz, col = log10(I)), size = 0.7) + 
...
Let's try a rd plot with plotly

```{r}
mytibble %>% 
  unnest(c("mz","I")) %>% 
  filter(mz > 380 & mz < 390) %>% 
  filter(rt > 2600 & mz < 2800) %>% 
  plot_ly() %>%
  add_markers(x = ~mz, y = ~rt, z = ~I, color = ~I, marker = list(size = 1))
```
...
chromPeaks(xdata) %>% 
  as_tibble() %>% 
  ggplot() + 
  geom_point(aes(x = rt, y = mz, col = into), siaze = 0.3) + 
  facet_wrap(~sample) + 
  theme_light()
```
