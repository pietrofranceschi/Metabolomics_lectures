...
plot(bpi, col = mypalette[sampleData(raw_data)$color])  ## raw_data$color is getting out the phenodata column called color
legend("topright", legend = c("red","Blank & QC","white"), col = mypalette, lty = 1)
```
```{r}
plot(bpi, col = mypalette[sampleData(raw_data)$color], xlim = c(50,200))
```
...
```{r}
plot(chr_raw[[24]], ylim = c(0,3000000))
```
