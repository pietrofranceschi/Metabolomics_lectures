...
```{r}
rt <- rtime(spectra(raw_one))

raw_spectra <- spectra(raw_one)

## we create a tibble of spectra ...
spectra_tibble <- tibble(spc_id = 1:length(rt)) %>% 
  mutate(rt = map_dbl(spc_id, ~ rt[.x])) %>% 
  mutate(data = map(spc_id, ~ tibble(mz = mz(raw_spectra[.x])[[1]],
                                     I = intensity(raw_spectra[.x])[[1]]))
  )
...
Here we see the peak maps of the different files. It is clear that each map is expected to be slightly different, but rally outlying samples will show up clearly. Look to the blanks (i.e., samples `r which(raw_data$variety == "blank")`), for example, or to the horizontal lines which are present in many files.
