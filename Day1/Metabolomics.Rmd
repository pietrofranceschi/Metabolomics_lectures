...
> 4. With an high **dynamic range** - able to measure at the same time low and high and low abundant compounds
...
### 3 - Matrix effects
In metabolomics we are loading on the instruments a complex mixture of chemicals. this complex environment will affect the _response_ of the molecules in a complex and often unpredictable way. The importance of this effect is so strong in MS that this technique is almost always coupled with chromatography in metabolomics
...
### Untargeted
We "throw" a raw extract in our instrument trying to analyze as much metabolites as possible. How many things we are able to see is unknown. 

_The measured variables are **noy ** metabolites_
...
Remember that instrument are ~~sensitive~~ so errors will unavoidably show up in the final data


## Annotation in Untargeted Metabolomics

As you have already understood, in metabolomics (untargeted)

- We do not measure the concentration of metabolites
- If I have the metabolite is easier to understand how to get the corresponding variables (ions, NMR peaks, features)
- ... the reverse (called ~~annotation~~) is not at all trivial

_~~Most likely, chemical annotation is the current bottleneck of metabolomics~~_
...
## Key Elements to design a succesfull metabolomics {.build}
