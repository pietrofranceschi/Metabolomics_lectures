...
Ok suppose now that I'm interested in estimating well the population mean. 
In my case this parameter is known ...
...
Suppose also that I do not have money to cover the full population, so the best I can do is to measure 30 people. So I'll use their mean height to estimate the mean of the population.  
...
As we can see, the estimate has variability (:-P), and it is centered around the correct value for the population mean. It is also normally distributed
...
```{r}
## this plot is made by using "base" R plotting utilities
hist(mypop)
abline(v = mean(mypop), 
       col = "red", 
