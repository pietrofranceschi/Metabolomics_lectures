...
mypop_m <- rnorm(50000,180,10)  ## Extracting 8000 numbers from a gaussian population with mean 180 and sd 10
...
Now I have my population, I want now to simulate the sdampling from that population. So basically I want to estimate the mean of my population from a group of samples extracted from `mypop`
...
Trust me, the variability of the estimate of the mean, decreases as the size of the sample increases. The distribution of the means (called sampling distribution of the mean) is normally distribute.


What to do now is to compare the spread of the estimates of the population mean when samplin gis performed in a "ranndom" fashion or in  a stratified fashion
...
sample_size_effect <- tibble(s_size = rep(c(sizes), each = 500)) %>% 
  mutate(m_fraction = length(mypop_m)/(length(mypop_m) + length(mypop_f))) %>% 
