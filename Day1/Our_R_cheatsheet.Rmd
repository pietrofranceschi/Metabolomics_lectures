...
## Homework
Create a tibble that contains 100 random sampling from a normal distribution (mu = 0, sd = 1 ... rnorm())
...
```{r}
your_tibble <- tibble(sample = Sample)
...
your_tibble <- your_tibble %>% 
  mutate(random = map(sample, function(x) runif(10)))
...
```{r}
your_tibble <- your_tibble %>% 
  mutate(means = map_dbl(random, function(pippo) mean(pippo)))
...
```{r}
hist(your_tibble$means)
```


## Homework 2

Show that the means of the 100 samples shows a normal distribution regardless the shape of the distribution we are drawing from
