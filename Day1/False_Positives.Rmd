...
Just to see what's going on let's give a look to one of the correlation matrices, actually the first 100 variables
...
```{r}
highcorrid <- which(dummy_data$uptri[[1]] > 0.95 & dummy_data$varCorr[[1]] != 1, arr.ind = TRUE)
highcorrid
```
...
0. Go back to the code I used to plot the correlation matrix and try to rewrite the nested function by using pipes ...
