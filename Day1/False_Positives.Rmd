...
This first demo deals with "random organization". As we discussed in the lecture, an experiment is always performed on a subset of the population, so a measured "organization" 

* can mirror a population level property
* can be the result of a "unlucky" sampling

To understand the second point, just consider an hypothetic sampling of the human population made by an extraterrestrial entity. If he/she (it?) random sample a couple of humans, it will likely conclude that we are all chinese! 

Let's make a more scientific example. Let's create a fake metabolomics experiment (but it can be everything you like ...) where I'm measuring the concentration of 1000 variables on a group of samples. And let's suppose that I want to study the correlation between the different variables to infer new regulatory networks ...


To simulate my dummy experiment I will fill my data matrices with random numbers, to show the role of the sample/variable ratio in this business I'll also simulate may dummy data with different number of samples

```{r}
## Random expression data matrix

dummy_data <- tibble(nsamples = seq(10,500,10)) %>% 
  mutate(DM = map(nsamples,~matrix(rnorm(1000*.x), ncol = 1000)))
...
dummy_data <- dummy_data %>% 
  mutate(varCorr = map(DM,~cor(.x)))
```

Just to see what's going on let's give a look to one of the correlation matrices
...
par(pty="s")
image(t(dummy_data$varCorr[[3]][1:100,100:1]), xaxt = "n", yaxt = "n", col = jet.colors(20))
```
...
The diagonal is made up of !s, because the correlation between each variable and itself is 1. The matrix is symmetric, since the measure of Pearson correlation is symmetric
...
Let's now start from the most "unfavorable" situation: 10 samples and 1000 variables
...
Unexpectedly ( ;-) ) ... we also have high correlation variables! The vertical lines show the 0.8 limit ... 
...
Before giving the chance to you to play with some random science, let's look oh what happens to the histogram of the correlation coefficients as the number of samples increases ...

```{r}
dummy_data %>% 
  filter(nsamples %in% c(10,100,300, 500)) %>% 
  select(nsamples, uptri) %>% 
  unnest(uptri) %>% 
  ggplot() + 
