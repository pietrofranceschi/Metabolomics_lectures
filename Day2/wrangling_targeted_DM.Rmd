...
PC_12_scores <- facto_summarize(myPCA, "ind", axes = c(1,2)) %>%    ## get the data from the PCA object
  mutate(name = as.character(name)) %>%              ## transform the sample name to a character
  rename("MouseID" = "name") %>%     ## align the name with the correct name
...
PC_12_scores %>% 
  ...
  geom_point(aes(x = Dim.1, y = Dim.2, pch = Gender), col  ="red") + 
